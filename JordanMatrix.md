...
В силу второго свойства жордановых цепочек векторы $e^{(0)}, ..., e^{(h-1)}$ образуют базис в $\mathcal{Y}$ и матрица ограничения имеет вид:
...
**Определение 12**: базис из **теоремы 7** называют жордановым базисом

# Нахождение корневых векторов

Собственные векторы с которых начинаются цепочки
